...
export async function GET(req: NextRequest, { params }: Params) {
  try {
    ...
    const activity = await getActivityHandler(ctx, activityId, farmId);
...
export async function PATCH(req: NextRequest, { params }: Params) {
  try {
    ...
    const parsed = UpdateActivityInputSchema.safeParse(body);
    if (!parsed.success) {
      throw new ApiError(422, "validation_error", firstError(parsed.error.issues, "Invalid input"));
    }
    const activity = await updateActivityHandler(ctx, activityId, farmId, parsed.data);
    return apiOk(activity);
  } catch (err) {
    return apiError(err);
  }
...
export async function DELETE(req: NextRequest, { params }: Params) {
  try {
    ...
    const hasAccess = await checkFarmAccess(farmId, ctx.userId);
    if (!hasAccess) throw new ApiError(403, "forbidden", "Access denied.");
    await deleteActivityHandler(ctx, activityId, farmId);
