...
import { CreateActivityInputSchema } from "@/features/activities/schema";
import { listActivitiesHandler, createActivityHandler } from "@/features/activities/handlers";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const activities = await listActivitiesHandler(ctx, farmId);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const activity = await createActivityHandler(ctx, farmId, parsed.data);
