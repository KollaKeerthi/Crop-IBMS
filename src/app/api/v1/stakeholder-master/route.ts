...
import { CreateStakeholderInputSchema } from "@/features/stakeholder-master/schema";
import {
  listStakeholdersHandler,
...
export async function GET(req: NextRequest) {
  try {
    ...
    const stakeholders = await listStakeholdersHandler(ctx, farmId);
    return apiOk(stakeholders);
  } catch (err) {
    return apiError(err);
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = CreateStakeholderInputSchema.safeParse(body);
    if (!parsed.success) {
      throw new ApiError(422, "validation_error", firstError(parsed.error.issues, "Invalid input"));
    }
    const stakeholder = await createStakeholderHandler(ctx, farmId, parsed.data);
    return apiOk(stakeholder, 201);
  } catch (err) {
    return apiError(err);
  }
}
