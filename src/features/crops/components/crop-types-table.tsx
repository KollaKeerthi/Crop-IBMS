      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-5">
        ...
        <FormField control={form.control} name="name" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Input {...field} placeholder="e.g. Cherry, Beefsteak, Roma" autoFocus /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
          ...
          <FormItem>
            ...
            <FormControl>
              <div className="relative">
                ...
                />
                <Input value={field.value ?? ""} onChange={(e) => field.onChange(e.target.value)} placeholder="e.g. #FF5733 or Red" className="pl-10" />
              </div>
            </FormControl>
            <FormMessage />
          </FormItem>
        ...
        <FormField control={form.control} name="description" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Input {...field} value={field.value ?? ""} placeholder="Brief description (optional)" /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
...
function EditTypeForm({
  ...
  const form = useForm<UpdateCropTypeInput>({
    ...
    defaultValues: { name: initial.name, colour: initial.colour ?? "", description: initial.description ?? "" },
  });
  const watchedColour = form.watch("colour");
...
export function CropTypesTable() {
  ...
  const [deletingItem, setDeletingItem] = useState<{ cropId: string; typeId: string; typeName: string } | null>(null);

  async function handleCreate(values: CreateCropTypeInput & { cropId: string }) {
    try {
      await createMutation.mutateAsync({ cropId: values.cropId, input: { name: values.name, colour: values.colour, description: values.description } });
  ...
  async function handleUpdate(values: UpdateCropTypeInput) {
    ...
    try {
      await updateMutation.mutateAsync({ cropId: editingItem.cropId, typeId: editingItem.id, input: values });
  ...
  async function handleDelete() {
    ...
    try {
      await deleteMutation.mutateAsync({ cropId: deletingItem.cropId, typeId: deletingItem.typeId });
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
                  ...
                  <TableCell>
                    {t.colour ? (
                      <div className="flex items-center gap-2">
                        <span className="h-4 w-4 rounded-full border border-border shrink-0" style={{ background: t.colour }} />
                        <span className="text-xs font-mono text-muted-foreground">{t.colour}</span>
                      </div>
                    ) : <span className="text-xs text-muted-foreground">-</span>}
                  </TableCell>
                  <TableCell className="text-muted-foreground max-w-[240px] truncate">{t.description ?? "-"}</TableCell>
                  <TableCell>
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => setDeletingItem({ cropId: t.cropId, typeId: t.id, typeName: t.name })}
        ...
        <EmptyState
          ...
          description={search ? "Try adjusting your search terms." : "Add the specific types or classifications of crops you grow."}
          action={!search && crops.length > 0 ? <Button onClick={() => setCreateOpen(true)}><Plus className="mr-2 h-4 w-4" />Add Type</Button> : undefined}
        />
      )}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>New Crop Type</DialogTitle></DialogHeader>
          <CreateTypeForm crops={crops} onSubmit={handleCreate} onCancel={() => setCreateOpen(false)} submitting={createMutation.isPending} />
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!editingItem} onOpenChange={(o) => !o && setEditingItem(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Edit Crop Type</DialogTitle></DialogHeader>
          {editingItem && (
            <EditTypeForm initial={editingItem} onSubmit={handleUpdate} onCancel={() => setEditingItem(null)} submitting={updateMutation.isPending} />
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!deletingItem} onOpenChange={(o) => !o && setDeletingItem(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Delete Crop Type</DialogTitle></DialogHeader>
          <p className="text-sm text-muted-foreground">
            Are you sure you want to delete{" "}
            <span className="font-semibold text-foreground">&quot;{deletingItem?.typeName}&quot;</span>?
