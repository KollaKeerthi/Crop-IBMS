      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-5">
        ...
        <FormField control={form.control} name="name" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Input {...field} placeholder="e.g. Red Robin, Early Girl" autoFocus /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
        <FormField control={form.control} name="gender" render={({ field }) => (
          <FormItem>
            ...
            <Select value={field.value ?? ""} onValueChange={(v) => field.onChange(v || undefined)}>
              <FormControl><SelectTrigger><SelectValue placeholder="Select gender (optional)" /></SelectTrigger></FormControl>
              <SelectContent>
                <SelectItem value="Male"><span className="flex items-center gap-2"><Mars className="h-3.5 w-3.5 text-blue-600" /> Male</span></SelectItem>
                <SelectItem value="Female"><span className="flex items-center gap-2"><Venus className="h-3.5 w-3.5 text-pink-600" /> Female</span></SelectItem>
              </SelectContent>
            </Select>
            <FormMessage />
          </FormItem>
        )} />
        <FormField control={form.control} name="colourDescription" render={({ field }) => (
          <FormItem>
            <FormLabel>Colour Description</FormLabel>
            <FormControl><Input {...field} value={field.value ?? ""} placeholder="e.g. Deep red, Yellow-green gradient" /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
...
export function CropVarietiesTable() {
  ...
  const [deletingItem, setDeletingItem] = useState<{ cropId: string; varietyId: string; varietyName: string } | null>(null);

  async function handleCreate(values: CreateCropVarietyInput & { cropId: string }) {
    try {
      await createMutation.mutateAsync({ cropId: values.cropId, input: { name: values.name, gender: values.gender, colourDescription: values.colourDescription } });
  ...
  async function handleUpdate(values: UpdateCropVarietyInput) {
    ...
    try {
      await updateMutation.mutateAsync({ cropId: editingItem.cropId, varietyId: editingItem.id, input: values });
  ...
  async function handleDelete() {
    ...
    try {
      await deleteMutation.mutateAsync({ cropId: deletingItem.cropId, varietyId: deletingItem.varietyId });
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
            ...
            <TableBody>
              {filteredVarieties.map((v) => (
                <TableRow key={v.id}>
                  ...
                  <TableCell><GenderBadge gender={v.gender} /></TableCell>
                  <TableCell className="text-muted-foreground">{v.colourDescription ?? "-"}</TableCell>
                  <TableCell>
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="ghost" size="icon" onClick={() => setEditingItem(v)} aria-label={`Edit variety ${v.name}`}>
                        <Pencil className="h-4 w-4 text-muted-foreground" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => setDeletingItem({ cropId: v.cropId, varietyId: v.id, varietyName: v.name })} aria-label={`Delete variety ${v.name}`}>
        ...
        <EmptyState
          ...
          description={search ? "Try adjusting your search terms." : "Add the specific varieties and cultivars of crops you grow."}
          action={!search && crops.length > 0 ? <Button onClick={() => setCreateOpen(true)}><Plus className="mr-2 h-4 w-4" />Add Variety</Button> : undefined}
        />
      )}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>New Crop Variety</DialogTitle></DialogHeader>
          <CreateVarietyForm crops={crops} onSubmit={handleCreate} onCancel={() => setCreateOpen(false)} submitting={createMutation.isPending} />
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!editingItem} onOpenChange={(o) => !o && setEditingItem(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Edit Crop Variety</DialogTitle></DialogHeader>
          {editingItem && (
            <EditVarietyForm initial={editingItem} onSubmit={handleUpdate} onCancel={() => setEditingItem(null)} submitting={updateMutation.isPending} />
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!deletingItem} onOpenChange={(o) => !o && setDeletingItem(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Delete Crop Variety</DialogTitle></DialogHeader>
          <p className="text-sm text-muted-foreground">
            Are you sure you want to delete{" "}
            <span className="font-semibold text-foreground">&quot;{deletingItem?.varietyName}&quot;</span>?
