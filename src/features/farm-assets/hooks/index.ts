...
export function useFarmAssets(farmId: string | null) {
  return useQuery({ queryKey: farmId ? key(farmId) : ["farm-assets", null], queryFn: () => listFarmAssets(farmId!), enabled: !!farmId });
}

export function useCreateFarmAsset() {
  const qc = useQueryClient();
  return useMutation({ mutationFn: createFarmAsset, onSuccess: (_, v) => qc.invalidateQueries({ queryKey: key(v.farmId) }) });
}

export function useUpdateFarmAsset() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, farmId, input }: { id: string; farmId: string; input: UpdateFarmAssetInput }) =>
      updateFarmAsset(id, farmId, input),
    onSuccess: (data) => { if (data) qc.invalidateQueries({ queryKey: key(data.farmId) }); },
  });
}
