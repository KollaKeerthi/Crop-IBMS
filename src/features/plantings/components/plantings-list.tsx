...
function findMatchingActiveTime(planting: Planting, activeTimes: ActiveTime[]): ActiveTime | undefined {
...
function getPlanActivityIcon(name: string): string {
  ...
  if (n.includes("weed") || n.includes("mulch") || n.includes("soil") || n.includes("compost")) return "🌾";
  if (n.includes("fertilize") || n.includes("feed") || n.includes("nutrient") || n.includes("fertiliser")) return "🧪";
  if (n.includes("prune") || n.includes("thin") || n.includes("stake") || n.includes("tie") || n.includes("trellis")) return "✂️";
  if (n.includes("pest") || n.includes("spray") || n.includes("treat") || n.includes("bug") || n.includes("disease")) return "🛡️";
...
function getPlanActivityColor(name: string): string {
  ...
  if (n.includes("fertilize") || n.includes("feed") || n.includes("nutrient") || n.includes("fertiliser")) {
  ...
  if (n.includes("prune") || n.includes("thin") || n.includes("stake") || n.includes("tie") || n.includes("trellis")) {
  ...
  if (n.includes("pest") || n.includes("spray") || n.includes("treat") || n.includes("bug") || n.includes("disease")) {
...
export function PlantingsList() {
  ...
  useEffect(() => {
    async function loadIntegrations() {
      try {
        const data = await apiFetch<{ provider: string; connectedAt: string }[]>("/api/v1/integrations/calendar/status");
  ...
  const activeDateEvents = selectedDate
    ? plantings.flatMap((p) => {
        ...
        if (isSameDay(selectedDate, p.nurseryStartDate)) {
          events.push({ planting: p, type: "Nursery Start", icon: "⏳", details: `Nursery propagation begins for ${p.cropName} (${p.varietyName ?? "Unknown variety"}).` });
        }
        if (isSameDay(selectedDate, p.fieldPlantingDate)) {
          events.push({ planting: p, type: "Field Planting", icon: "🌱", details: `Transplanting ${p.cropName} to field blocks.` });
        }
        if (isSameDay(selectedDate, p.firstHarvestDate)) {
          events.push({ planting: p, type: "Harvest Start", icon: "🧺", details: `First estimated harvest window begins for ${p.cropName}.` });
        }
        if (isSameDay(selectedDate, p.harvestEndDate)) {
          events.push({ planting: p, type: "Harvest End", icon: "✅", details: `Final harvest window concludes for ${p.cropName}.` });
        }
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative z-10">
            ...
            {integrations.length === 0 && (
              <Link href="/dashboard/settings/integrations">
                <Button size="xs" variant="outline" className="border-amber-500/20 text-amber-600 hover:bg-amber-500/10 hover:border-amber-500/40 text-xs shrink-0 cursor-pointer shadow-3xs">
                  Connect Services
                </Button>
              </Link>
            )}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between bg-card p-4 rounded-xl border shadow-2xs">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <Button variant="outline" size="sm" className="h-8 text-xs font-semibold cursor-pointer" onClick={() => setCalDate(new Date())}>
                Today
              </Button>
              ...
              {currentMonthDays.map((day, idx) => {
                ...
                const dayEvents = plantings.flatMap((p) => {
                  ...
                  if (isSameDay(day.date, p.nurseryStartDate)) {
                    events.push({ type: "Nursery", label: `${p.cropName} (Nursery)`, icon: "⏳", color: "bg-amber-500/10 text-amber-600 border-amber-500/20 hover:bg-amber-500/20" });
                  }
                  if (isSameDay(day.date, p.fieldPlantingDate)) {
                    events.push({ type: "Planting", label: `${p.cropName} (Planting)`, icon: "🌱", color: "bg-emerald-500/10 text-emerald-600 border-emerald-500/20 hover:bg-emerald-500/20" });
                  }
                  if (isSameDay(day.date, p.firstHarvestDate)) {
                    events.push({ type: "Harvest", label: `${p.cropName} (Harvest)`, icon: "🧺", color: "bg-sky-500/10 text-sky-600 border-sky-500/20 hover:bg-sky-500/20" });
                  }
                  ...
                  <div
                    ...
                    onClick={() => {
                      if (dayEvents.length > 0) {
                        ...
                        toast.info(`No planting events scheduled for ${day.date.toLocaleDateString()}`);
                      }
                    }}
                    className={`min-h-[110px] p-2 flex flex-col justify-between bg-card transition-colors select-none ${
                      day.isCurrentMonth ? "text-foreground" : "text-muted-foreground/30 bg-muted/10"
                    ...
                    <div className="flex justify-between items-start">
                      <span className={`text-[11px] font-bold rounded-full w-5 h-5 flex items-center justify-center ${
                        isToday 
                          ...
                          : day.isCurrentMonth ? "text-foreground/80" : "text-muted-foreground/20"
                      }`}>
                        {day.date.getDate()}
                      </span>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/40">
                  <th className="px-4 py-3.5 text-left font-semibold text-muted-foreground text-xs uppercase tracking-wider">Crop</th>
                  <th className="px-4 py-3.5 text-left font-semibold text-muted-foreground text-xs uppercase tracking-wider">Variety</th>
                  <th className="px-4 py-3.5 text-left font-semibold text-muted-foreground text-xs uppercase tracking-wider">Season</th>
                  <th className="px-4 py-3.5 text-left font-semibold text-muted-foreground text-xs uppercase tracking-wider">Status</th>
                  <th className="px-4 py-3.5 text-left font-semibold text-muted-foreground text-xs uppercase tracking-wider">Nursery Start</th>
                  <th className="px-4 py-3.5 text-left font-semibold text-muted-foreground text-xs uppercase tracking-wider">Planting Date</th>
                  <th className="px-4 py-3.5 text-left font-semibold text-muted-foreground text-xs uppercase tracking-wider">Harvest Start</th>
                  <th className="px-4 py-3.5 text-left font-semibold text-muted-foreground text-xs uppercase tracking-wider">Harvest End</th>
                  ...
                  <tr key={p.id} className="hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center justify-end gap-1">
                        <Button variant="ghost" size="icon" onClick={() => setEditPlanting(p)} className="cursor-pointer">
                          <Pencil className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => setDeletingId(p.id)} className="cursor-pointer">
        ...
        <DialogContent className="sm:max-w-md rounded-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-base font-bold">
              <Calendar className="h-5 w-5 text-primary" />
              Events: {selectedDate?.toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
            </DialogTitle>
          </DialogHeader>
            ...
            {activeDateEvents.length > 0 ? (
              <div className="space-y-3">
                {activeDateEvents.map((ev, idx) => (
                  <div key={idx} className="rounded-xl border p-4 bg-muted/20 flex flex-col gap-2 transition-shadow hover:shadow-2xs">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-base">{ev.icon}</span>
                        <span className="text-xs font-bold uppercase tracking-wider text-foreground">{ev.type}</span>
                      </div>
